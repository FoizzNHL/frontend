...
def draw_bitmap(bitmap_rows, x0, y0, color=ON):
    # bitmap_rows = list[str], each str is "010101"
...
def draw_number(n: int, color=ON, gap=1):
    """
    Draw 1-2 digit number centered on 15x12.
    (For NHL player numbers, 0-99 is perfect.)
    """
    s = str(n)
    ...
    if len(s) == 1:
        draw_bitmap(DIGITS_6x9[s], x0, y0, color)
    else:
        d1, d2 = s[0], s[1]
        draw_bitmap(DIGITS_6x9[d1], x0, y0, color)
        draw_bitmap(DIGITS_6x9[d2], x0 + digit_w + gap, y0, color)

    strip.show()

if __name__ == "__main__":
    print("Testing 6x? digits on 15x12 matrix (Ctrl+C to stop)")
    try:
        # Demo sequence — change to your own player numbers
        players = [13, 14, 22, 27, 40, 73, 88, 8, 9, 0]
        while True:
            for p in players:
                print("Showing:", p)
                draw_number(p, color=ON, gap=1)
                time.sleep(1.5)
